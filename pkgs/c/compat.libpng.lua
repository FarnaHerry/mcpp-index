-- compat.libpng — PNG reference library (libpng 1.6.43).
-- ~15 C source files, depends on compat.zlib.
-- EUI-NEO uses libpng for PNG image encoding/decoding.
--
-- Key: pnglibconf.h is NOT in the upstream tarball; it is generated at CMake
-- time from scripts/pnglibconf.h.prebuilt. We ship the prebuilt version as a
-- mcpp generated_file so the build is self-contained without CMake.
--
-- All `mcpp` paths are GLOBS relative to the verdir; the leading `*/`
-- absorbs the GitHub tarball's `libpng-1.6.43/` wrap layer.
package = {
    spec        = "1",
    namespace   = "compat",
    name        = "libpng",
